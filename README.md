...
**Mismatch Ratio Limit** - (**DEFAULT**: 0.2; *e* in algorithmic description below) this parameter must be a number between 0 and 1 specified as decimal. A low value for this parameter permits fewer mismatches relative to the current expansion *E* of the telomere at any step during the algorithm, while a high value allows for more mismatches. Setting the value of this parameter to 0, effectively forbids mismatches and leads to the discontinuation of the telomere expansion as soon as one mismatch is encountered. Setting the value of this parameter to 1 ignores mismatches while expanding the telomere as long as all other conditions are met.

**Limit of Excess Mismatches in Buffer** - (**DEFAULT**: 4; *h* in algorithmic description below) this parameter must be a nonnegative integer. Generally, a low value for this parameter permits fewer mismatches close together without being "balanced out" by basepairs conforming to the ideal telomere. Setting the value of this parameter to 0, effectively forbids mismatches and leads to the discontinuation of the telomere expansion as soon as one mismatch is encountered. Setting the value of this parameter to a high number allows for longer substrings of the telomere to deviate from the ideal telomere.

**Maximum Length** - (**DEFAULT**: 100; *l* in algorithmic description below) this parameter must be a nonnegative integer. The telomere cannot be expanded to a sequence longer than the value of this parameter. If the combined length of the buffer *B* and the current expansion *E* of the telomere exceed this maximum length, the buffer is discarded and expansion is halted. If the value of this parameter is set to an integer less than the length of the *Telomere Pattern*, the algorithm will never return a telomere.

**Maximum Distance to Sequence End** - (**DEFAULT**: 100; *o* in algorithmic description below) this parameter must be a nonnegative integer. The distance between the telomere and the corresponding end of the sequence cannot exceed the value of this parameter. Setting the value of this parameter to 0 will forbid the presence of nontelomeric portions at those ends of the sequence where telomeres were found and lead to the exclusion of any such telomeres from the telomere annotation.

**Minimum Length** - (**DEFAULT**: 12; *m* in algorithmic description below) this parameter must be a nonnegative integer. Any telomeric sequence found by the algorithm which does not have a length of at least the value of this parameter is excluded from the algorithm. If the value of this parameter is set to a value less than or equal to the length of the *Telomere Pattern*, the algorithm will only return telomeres at least as long as the *Telomere Pattern*.
...
**Minimum HSP Length for Match Annotation** - (*l* in algorithmic description below) this parameter must be a positive integer. The value of this parameter determines the minimum length a high-scoring pair provided by BLAST must have to be considered for match annotation.

**Minimum Bitscore for Preliminary Match Annotation** - (*b* in algorithmic description below) this parameter must be a nonnegative integer. The value of this parameter determines the minimum bitscore a high-scoring pair provided by BLAST must have to be considered for preliminary match annotation.

**Minimum Percent Identity for Preliminary Match Annotation** - (*q* in algorithmic description below) this parameter must be a number between 0 and 100. The value of this parameter determines the minimum percent identity a high-scoring pair provided by BLAST must have to be considered for preliminary match annotation.

**Minimum Additional Base Pair Coverage to Qualify for Preliminary Match Annotation** - (*c* in algorithmic description below) this parameter must be a nonnegative integer. The value of this parameter determines the number of base pairs a high-scoring pair provided by BLAST must cover, and which are not already covered by previously considered high-scoring pairs, or merged high-scoring pairs, to be considered for preliminary match annotation.

**Tolerance for Discrepancies Between Relative Positions of Precursor and Product Intervals for HSP Merging** - (*t* in algorithmic description below) this parameter must be a nonnegative integer. The value of this parameter determines the maximum discrepancy between the relative positions of the corresponding end points of the precursor and product regions of two high-scoring pairs provided by BLAST, for them to be considered for merging.

**Maximum Gap Length for HSP Merging** - (*g* in algorithmic description below) this parameter must be 

**Minimum Base Pair Length for Gap Annotation** - (*u* in algorithmic description below)

**Compute Pointer Annotations on Precursor and Product Sequences** - 

**Minimum Base Pair Length for Pointer Annotation** - (*v* in algorithmic description below)

**Minimum Bitscore for Additional Match Annotation** - (*b'* in algorithmic description below)

**Minimum Percent Identity for Additional Match Annotation** - (*q'* in algorithmic description below)

**Minimum Coverage of Product Interval for Additional Match Annotation** - (*r* in algorithmic description below)

**Minimum Coverage of Product Interval for Fragment Annotation** - (*s* in algorithmic description below)
